...
static constexpr const char* tmpFileBak = "/tmp/srvcfg-mgr.json.bak";
                            ...
                            const std::vector<ListUnitsType>& listUnits)
{
    ...
    bool jsonExist = fs::exists(srvCfgMgrFile);
    if (jsonExist)
    {
        try
        {
        ...
        catch (const std::exception& e)
        {
            ...
            std::error_code ec;
            fs::copy_file(srvCfgMgrFile, tmpFileBak,
                          fs::copy_options::overwrite_existing, ec);
            if (ec)
            {
                lg2::error(
                    "Failed to copy srvCfgMgrFile to tmpFileBak, srvCfgMgrFile = {SRCFILE}, tmpFileBak = {DSTFILE}",
                    "SRCFILE", srvCfgMgrFile, "DSTFILE", tmpFileBak);
            }
          ...
          std::shared_ptr<sdbusplus::asio::connection>& conn)
{
    ...
    conn->async_method_call(
        [&server, &conn](boost::system::error_code ec,
            ...
            {
                lg2::error(
                    "async_method_call error: ListUnits failed, ec = {EC}",
                    "EC", ec.value());
                return;
            }
        ...
        [&server, &conn](boost::system::error_code ec,
            ...
            else
            {
                ...
                initTimer->async_wait([&server, &conn](
                    ...
                    {
                        lg2::error(
                            "service config mgr - init - async wait error, ec = {EC}",
                            "EC", ec.value());
                        return;
                    }
