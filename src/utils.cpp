                                  ...
                                  const std::string& msg)
{
    if (ec)
    {
        ...
        lg2::error("Check and throw internal failure: message = {MSG}", "MSG",
                   msgToLog);
...
static inline uint32_t getJobId(const std::string& path)
{
    ...
    {
        lg2::error("Unable to get job id from job path");
        phosphor::logging::elog<
            sdbusplus::xyz::openbmc_project::Common::Error::InternalFailure>();
    }
                       ...
                       const std::string& actionMethod)
{
    ...
    while (true)
    {
        ...
        if (ec)
        {
            ...
            lg2::error("Systemd operation failed for job query");
